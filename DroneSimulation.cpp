...
DroneSimulation::DroneSimulation()
    : m_nh(DEFAULT_PORT), m_doLoop(true), m_config(nullptr)
{
}
...
std::map<std::string, Command*> DroneSimulation::createCommandsMap(DroneState& current)
{
    ...
    auto rawForward = new Forward(&m_world, &current.getPosNonConst(), *current.getCompassPtr());
    auto rawLeft = new TurnLeft(current.getCompassPtr());
    auto rawRight = new TurnRight(current.getCompassPtr());

    commands["map"] = new CmdShowMap(&m_world, &current.getPos(), activePositions);
    commands["fwd"] = new CmdFixeConsumption(rawForward, 10);
    commands["+90"] = new CmdFixeConsumption(rawRight, 5);
    commands["-90"] = new CmdFixeConsumption(rawLeft, 5);
    ...
    if (m_config->povSensor)
    {
        ...
        commands["pov"] = new CmdFixeConsumption(rawPov, m_config->povSensor->getEnergyCost());
    }
    else
    {
...
void DroneSimulation::handleCollisions(size_t activeId)
{
    ...
    for (auto& [id, other] : m_drones)
    {
            ...
            otherPos.getY() == curPos.getY())
        {
            ...
            m_nh.PutOutput(id, "CRASH! Drone " + std::to_string(activeId) + " collided into you!\n");
            break;
        }
    }
}
...
void DroneSimulation::resolveAndExecuteCommand(size_t id, DroneState& current, const std::string& input)
{
    ...
    {
        m_nh.PutOutput(id, "[ERROR] Unknown command: '" + cleanS + "'.\n");
        executeCommand(id, current, CMD_UNKNOWN);
    }

    for (auto& pair : commands)
    {
        if (pair.second != &CMD_UNKNOWN) delete pair.second;
    }
}
